...
exports.show = async (req, res, next) => {
    try {
        ...
        if (!goal) return res.status(404).render('error', { title: 'Not Found', message: 'Goal not found', error: {} });
...
exports.create = async (req, res, next) => {
    ...
    if (!errors.isEmpty()) return res.status(422).render('goals/new', { title: 'New Goal', errors: errors.array() });

    try {
        const { title, description, targetDate } = req.body;
        const { id } = await Goal.create({ userId: req.session.user.id, title, description, targetDate });
...
exports.update = async (req, res, next) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) return res.status(422).render('goals/edit', { title: 'Edit Goal', errors: errors.array(), goal: { ...req.body, id: req.params.id } });
